...
class FourWheeledCar(CSVRobot):
    ...
    def create_message(self):
        ...
        def detect_lanes(img, height, width):
            ...
            for y in range(roi_y, height, 5):
                ...
                for x in range(width):
                    # Get pixel color (assuming white lanes on dark background)
                    r = self.camera.imageGetRed(img, width, x, y)
                    g = self.camera.imageGetGreen(img, width, x, y)
                    b = self.camera.imageGetBlue(img, width, x, y)
        
                    # Check if pixel is bright (part of a lane marking)
        ...
        # print(box_detected, pedestrian_detected)
        ...
        message = [str(self.distance_sensor.getValue()), # get Vaีlue from distance sensor
                   ...
                   str(self.wheels_rear_left_position_sensor.getValue()), str(self.wheels_rear_right_position_sensor.getValue()),
    ...
    def use_message_data(self, message):
        ...
        assert action == 0 or action == 1 or action == 2 or action == 3, "4 Wheeled Car controller gets incorrect action value:" + str(action)
