...
def save_loss_plot(actor_loss_data, critic_loss_data, ppo_update_iter, episode):

    file_path = os.path.join(TEST_RESULT_DIR, 'loss', f'PPO Loss episode {episode + 1}.csv')

    if os.path.exists(file_path):
        df = pd.read_csv(file_path)

        new_data = pd.DataFrame({'ppo_update_iter': [ppo_update_iter], 'Actor_loss': [actor_loss_data], 'Critic_loss': [critic_loss_data]})
        df = pd.concat([df,new_data], ignore_index=True)

    else:
        df = pd.DataFrame({'ppo_update_iter': [ppo_update_iter], 'Actor_loss': [actor_loss_data], 'Critic_loss': [critic_loss_data]})
...
def save_transition_plot(state, actions, action_prob,episode):
    data = {
        'State': state,
        'Action': actions,
    ...
    file_path = os.path.join(TEST_RESULT_DIR, 'state', f'transitions{episode + 1}_plot.csv')
    df.to_csv(file_path, index=False)
...
def save_x_y_plot(x_y,episode):
    ...
    df = pd.DataFrame(data)
    file_path = file_path = os.path.join(TEST_RESULT_DIR, 'coordinates', f'coordinates{episode + 1}_plot.csv')
    df.to_csv(file_path, index=False)
